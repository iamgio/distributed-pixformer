...
- When the client attempts a connection to a server and it fails, it sets the server up and connects to it again.  
  By doing so, the client is the leader;
...
An alternative that was thoroughsly considered was a peer-to-peer architecture which consisted of one server
...
On top of that, the system adopts a ***client-side prediction and server reconciliation*** pattern:
...
- Entity mapping:
  - The game loop is performed both on client and server side,
    with the server periodically sending alignment updates to clients.
...
Clients send only event messages to the server. The server can send one of the two following messages to clients:
- Event messages received by a client, which are forwarded via broadcast to all the clients;
- A command to realign to the centralized game state, which is attached in the message body and contains:
